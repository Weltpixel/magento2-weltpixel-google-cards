...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @return mixed
     */
    public function getBreadcrumbType($storeId = 0)
    {
        if ($storeId) {
            return $this->scopeConfig->getValue('weltpixel_google_cards/general/breadcrumbs_type', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId) ?? '';
        } else {
            return $this->_cardsOptions['general']['breadcrumbs_type'] ?? '';
        }
    }

    /**
     * Remove widget tags from content
     *
     * @param string $content
     * @return string
     */
    public function stripWidgets($content)
    {
        if (empty($content)) {
            return $content;
        }

        // Remove widget tags: {{widget ...}}
        $pattern = '/\{\{widget[\s\S]*?\}\}/';
